...
class op_grade_calc_line(osv.osv):
    ...
    _defaults = {
        'use_new_student_assessment_line': True,
    }
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if vals.get("exam_score", False):
            ...
            details += " <br/> For Subject : " + grade_calc_id.subject_id.name
            subject = "Student Updated Values"
            self.pool.get("op.gradepoint").message_post(cr, uid, [grade_calc_id.gradepoint_gradecalc_id.id], body=details, subject=subject, limit=2, context=context)
...
class op_gradepoint(osv.osv):
    ...
    def calculate_grade(self, cr, uid, ids, context=None):
        uid = SUPERUSER_ID
        ...
        att_sheet_obj = self.pool.get('op.attendance.sheet')
        course_reg_pool = self.pool.get("course.registration")

        for self_obj in self.browse(cr, uid, ids, context=context):
            if self_obj.grade_calculation_per_sem_line:
                inner_ids = op_grade_calc_line_pool.search(cr, uid, [('active', '=', False),
                                                                     ('gradepoint_gradecalc_id', '=', self_obj.id)])
                if inner_ids:
                ...
                for gcpsl in self_obj.grade_calculation_per_sem_line:
                    if not gcpsl.use_new_student_assessment_line:
                        sl_ids = sl_pool.search(cr, uid, [('subject_id', '=', gcpsl.subject_id.id),
                                                          ('batch_id', '=', self_obj.semester_id.id)])
                        sl_ids = sl_ids and sl_ids[0] or False
                        if sl_ids:
                            sl_obj = sl_pool.browse(cr, uid, sl_ids, context=context)
                            grade_dict = {'exam_score': 0.0, 'attendance_score': 0.0, 'total_ca_score': 0.0,
                                          'total_score': 0.0}
                            assignment_sub_lst = []
                            ...
                            for aci in sl_obj.assessment_configuration_ids:
                                if aci.op_assessment_master_ids.ap_type == 'ca':
                                    ...
                                    assign_total_per = 0
                                    assignment_sub_ids = assign_sub_line_pool.search(cr, uid, domain, context=context)
                                    if assignment_sub_ids:
                                        assignment_sub_lst.extend(assignment_sub_ids)
                                        for asl_obj in assign_sub_line_pool.browse(cr, uid, assignment_sub_ids,
                                                                                   context=context):
                                            assign_total_per += asl_obj.percentage
                                        assign_score = assign_total_per * aci.percentage_weight / 100
                                        grade_dict['total_ca_score'] += assign_score
                                    grade_dict.update({
                                        'gradepoint_assignment_ids': [(6, 0, assignment_sub_lst)],
                                    })
                                ...
                                if aci.op_assessment_master_ids.ap_type == 'exam':
                                    ...
                                    op_exam_attendees_ids = op_exam_attendees_pool.search(cr, uid, domain, context=context)
                                    if op_exam_attendees_ids:
                                        for opaid in op_exam_attendees_ids:
                                            op_exam_rec = op_exam_attendees_pool.browse(cr, uid, opaid, context=context)
                                            exam_per += op_exam_rec.per
                                ...
                                if aci.op_assessment_master_ids.ap_type == 'attendance':
                                    tag_ids = self.pool.get('registered.subject').search(cr, uid, [
                                        ('subject_id', '=', gcpsl.subject_id.id), ('active', '=', 'True')])
                                    ## Get Attendace sheet id
                                    sheet_id = att_sheet_obj.search(cr, uid, [('subject_tag_id', 'in', tag_ids)])
                                    if sl_obj.attedence_selection == "Old":
                                        if sheet_id:
                                            ## Get registerd course ids
                                            app_course_ids = course_reg_pool.search(cr, uid, [('course_id', '=',sl_obj.course_id.id),
                                                                                     ('semester_id', '=',sl_obj.batch_id.id),('state', '=', 'approved')])
                                            for op_attendance_id in att_sheet_obj.browse(cr, uid, sheet_id):
                                                # for app_course_id in course_reg_pool.browse(cr, uid, app_course_ids):
                                                domain = [
                                                    ('student_id.id', '=', self_obj.student_id.id),
                                                    ('attendance_id', '=', op_attendance_id.id),
                                                ]
                                                line_ids = op_attendance_line_pool.search(cr, uid, domain, context=context)
                                                for line_id in line_ids:
                                                    op_attendance_line_ids.append(line_id)
                                                # op_attendance_line_ids = [line_id for line_id in line_ids]
                                        else:
                                            domain = [
                                                    ('student_id.id', '=', self_obj.student_id.id),
                                        ...
                                        if op_attendance_line_ids:
                                            ...
                                            for asid in op_attendance_line_ids:
                                                op_attendance_line_rec = op_attendance_line_pool.browse(cr, uid, asid,
                                                                                                        context=context)
                                        ...
                                        if op_att_line:
                                            ...
                                            if attendper >= sl_obj.base_attendence:
                                                attengread = (attendper - sl_obj.base_attendence) / (
                                                100 - sl_obj.base_attendence) * aci.percentage_weight
                                            ...
                                            #    grade_dict['attendance_score'] = 0
                                grade_dict.update({
                                    'gradepoint_exam_ids': [(6, 0, op_exam_attendees_ids)],
                            ...
                            raise except_orm(_('No Subject Line Found!'),
                                             _("You have to define subject line for subject '%s' !") % (
                                             gcpsl.subject_id.name,))
